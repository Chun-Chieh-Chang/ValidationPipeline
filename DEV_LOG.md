...
### 背景 (Background)
為解決任務清單與編輯詳情之間的邏輯斷層（如進度與實際日期欄位缺失），以及術語不統一產生的認知混淆，執行了深度的「數據鏈條對齊」與「術語標準化」工程。

### 核心變更 (Key Changes)
- **數據鏈條完整化 (Data Chain Integrity)**:
  - **TaskModal 強化**: 新增「進度 (%)」與「實際完成日」輸入欄位，消滅「孤兒欄位」。
  - **自動化連鎖反應**: 實作狀態、進度與日期之間的雙向連動邏輯（如：狀態設為已完成 -> 進度自動 100% -> 自動填入實際完成日）。
- **術語全域統一 (Terminology SSoT)**:
  - 將「負責部門/權責」統一為「**權責部門**」。
  - 將「工作狀態」統一為「**狀態**」。
  - 更新所有 Modal 標頭與 WBS 表格標頭，確保與輸入欄位 100% 吻合。
- **通知引擎整合**:
  - 重構 `triggerNextTaskNotifications` helper 函數，確保無論透過「快速狀態更新」或「彈窗完整儲存」，後續任務的接手提醒邏輯完全一致。
- **型別系統收斂**:
  - 將 `TaskData` 介面定義整合至 `projectService.ts`，移除分散各處的重複定義，確保系統強健性。
